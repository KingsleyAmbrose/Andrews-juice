...
app.get("/show all record", (req, res) => {
  ...
  dbo.collection('kcocollection').find().toArray(function(err, results) {
...
MongoClient.connect('mongodb+srv://user310:Password123@cluster0-jdkzc.mongodb.net/test?retryWrites=true&w=majority', function(err, db) {
  if(!err) {
  p = db
...
var u_price = req.body['price'];
  ...
  var myquery = { kcocollection: u_topic }; //record you want to search
  var newvalues = { $set: {topic:u_topic, location:u_location,  pirce:u_price } };


  // collection
  dbo2.collection("kcocollection").updateOne(myquery, newvalues, function(err, res) {
    if (err) throw err;
...
var u_topic = req.body['topic'];
  ...
  var myquery = { kcocollection: u_topic };
  var newvalues = { $set: {kcocollection: u_topic} };

  dbo2.collection("kcocollection").deleteOne(myquery, function(err, res) {
    if (err) throw err;
...
/* Add course */
  ...
  dbo1.collection('kcocollection').save({topic: u_topic, location:u_location ,price:u_price}, (err, result) => {
     if (err) return console.log(err)
